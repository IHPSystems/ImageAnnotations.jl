...
function ObjectDetectionCore(class_name::String, detector_type::DetectorType, detector_name::String)
    ObjectDetectionCore(class_name, nothing, nothing, nothing, detector_type, detector_name)
end

function ObjectDetectionCore(class_name::String, confidence::Float32, detector_type::DetectorType, detector_name::String)
    ObjectDetectionCore(class_name, confidence, nothing, nothing, detector_type, detector_name)
end
...
function centroid(detection::AbstractObjectDetection{T}) :: Point2{T} where T
    error("No implementation for $(typeof(detection))")
end
...
function iou(d1::AbstractObjectDetection{T}, d2::AbstractObjectDetection{T}) where T
    bb1 = bounding_box(d1)
    bb2 = bounding_box(d2)
    left_1, top_1 = bb1.origin; right_1, bottom_1 = bb1.origin + bb1.widths
    left_2, top_2 = bb2.origin; right_2, bottom_2 = bb2.origin + bb2.widths
